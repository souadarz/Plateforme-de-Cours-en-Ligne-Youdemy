                ...
                <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
                    ...
                    <div class="grid gap-4 gap-y-10 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                        <?php foreach ($categories as $category): ?> 
                            <div class="category_box flex flex-col rounded-lg border border-gray-200 bg-white px-4 py-4 hover:bg-gray-50" data-category-id="<?= $category['category_id']?>">

                                <div class="flex justify-between mb-5">
                                    <button class="edit_category_button">Edit</button>
                                    <!-- delete category -->
                                    <form action="/admin/deleteCategory" method="POST">
                                        <input type="text" value="<?= $category['category_id']?>" class="hidden" name="id_category">
                                        <input type="submit" value="Delete" name="delete_category" class="cursor-pointer">
                                    </form>
                                </div>

                                <h3 class="text-xl font-semibold text-gray-900 sm:text-2xl"><?= $category['category_name']?></h3>
                            </div>
                        <?php endforeach; ?>
